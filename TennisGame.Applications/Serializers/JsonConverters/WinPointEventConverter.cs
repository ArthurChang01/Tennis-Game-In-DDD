...
{
    public class WinPointEventConverter : JsonConverter<WinPointEvent>
    {
        public override WinPointEvent Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            while (reader.Read())
            {
                if (reader.TokenType == JsonTokenType.EndObject)
                    return new WinPointEvent(id, version, occuredDate, teamId, playerId, newScore, status);
        ...
        public override void Write(Utf8JsonWriter writer, WinPointEvent value, JsonSerializerOptions options)
        {
